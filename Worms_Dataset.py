...
params = {
    ...
    "batch_size": 1,  # the number of image per sample check if need to be changed -A: FOR NOW ITS FINE !
    "num_workers": 4,  # todo: relearn whats workers and ask what will be ours -A: itay explained NEED TO GO DEEPER in TRAINING MODEL !
    "image_size": 256,  #  to ask if this need to be the max from worms size padded up square or rectangle and how -A: FOR NOW FINE in future will need to be max padded!
...
# NEW TODO: 2) how to concat horizontal or vertical ?( with flag 0 or 1 torch.cat(image,label))?
####################################################
...
def make_paths_list(data_path):
    ...
    # todo: this is maybe problem because its dependet on the order of glob i can take only adress of dic and use
    #  replace do find the label in the folder regardless of order, it may be costly but can work (can run here on dic_paths or in every call to get
    dic_path_list = [dic_image for dic_image in mixed_images if dic_image.endswith("(DIC)_M0000_ORG.tif")]
    floresence_path_list = [floresence_image for floresence_image in mixed_images if floresence_image.endswith("(mCher)_M0000_ORG.tif")]
    return dic_path_list, floresence_path_list


dic_path_list, floresence_path_list  = make_paths_list(TRAIN_DATASET_PATH)
...
#show_dataset_paths(train_image_paths)
...
class WormsDataset(Dataset):
    def __init__(self, image_paths,labels_paths, transform=None):
        self.dic_images_paths = image_paths
        self.floresence_images_paths = labels_paths
        self.transform = transform

    def __len__(self):
        return len(self.dic_images_paths)

    def __getitem__(self, index):  # ask if its ok i am running over database class __getitem__ func? -A: its fine.
        dic_image_filepath = self.dic_images_paths[index]
        florescence_image_path = self.floresence_images_paths[index]
        # ask why cv2.imread() not referenced? saw pycharm error, and its working !-A:  Seems fine ignore it !
        dic_image = cv2.imread(dic_image_filepath, 0)  # 0 is grey flag
        florescence_image = cv2.imread(florescence_image_path, 0)
        if self.transform:
            dic_image = self.transform(dic_image)
            #NEW TODO: florescence_image = self.transform(florescence_image) to do this or to do concat and than?
        # NEW TODO: this label is mcher image in future will be florescence
        return dic_image, florescence_image
...
train_dataset = WormsDataset(dic_path_list, floresence_path_list, train_transform)
# x=train_dataset.__getitem__(0)
# print("this is dic\n",x[0])
# print()
# print("this is floresence\n",x[1])
...
def show_images_one_by_one(dataset):
    ...
    for sample in dataset:
        ...
        # lst.append(image)
        dic_image, florescence_image = sample
...
def show_in_grid(images_iter):
    ...
    for batch in images_iter:
        ...
        images, labels = batch
        grid = torchvision.utils.make_grid(images, nrow=2)  # padding=0,nrow and padding variables need to be adjust
